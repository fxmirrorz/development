...
-- tell oUF Framework that we use our own oUF version (ns.oUF, also know as X-oUF in /DuffedUI/DuffedUI.toc)
local ADDON_NAME, ns = ...
local oUF = ns.oUF
oUFDuffedUI = ns.oUF -- MOP BETA
...
D.PostCreateAura = function(self, button)
	...
	button.remaining = D.SetFontString(button, C["media"].font, C["unitframes"].auratextscale, "THINOUTLINE")
...
D.UpdateTargetDebuffsHeader = function(self)
	...
	h:SetPoint("BOTTOMLEFT", self, "BOTTOMLEFT", (D.lowversion and 0) or -2, y + addition)
end
...
D.UpdateReputationColor = function(self, event, unit, bar)
	local name, id = GetWatchedFactionInfo()
	bar:SetStatusBarColor(FACTION_BAR_COLORS[id].r, FACTION_BAR_COLORS[id].g, FACTION_BAR_COLORS[id].b)
end

-- when called, it update the name of unit
D.UpdateName = function(self,event)
