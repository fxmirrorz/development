...
if C["datatext"].dps_text and C["datatext"].dps_text > 0 then
	...
	local DPS_FEED = CreateFrame("Frame", "DuffedUIStatDamage")
	DPS_FEED.Option = C["datatext"].dps_text
	local player_id = UnitGUID("player")
	local dmg_total, last_dmg_amount = 0, 0
	local cmbt_time = 0
	DPS_FEED.Color1 = D.RGBToHex(unpack(C["media"].datatextcolor1))
	DPS_FEED.Color2 = D.RGBToHex(unpack(C["media"].datatextcolor2))
	G.DataText.DPS = DPS_FEED

	local pet_id = UnitGUID("pet")
     
	local dText = DPS_FEED:CreateFontString("DuffedUIStatDamageText", "OVERLAY")
	dText:SetFont(C["media"].font, C["datatext"].fontsize)
	dText:SetText("0.0 ",L.datatext_dps)
	G.DataText.DPS.Text = dText

	D.DataTextPosition(C["datatext"].dps_text, dText)
