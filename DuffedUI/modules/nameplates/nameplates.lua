...
local function CreateAuraIcon(parent)
	...
	button.count:SetFont(C["media"].font, C["datatext"].fontsize, "THINOUTLINE")
...
local function UpdateAuraIcon(button, unit, index, filter)
	...
	button.cd:SetScript("OnUpdate", function(self)
		...
		button.cd.timer.text:SetFont(C["media"].font, C["datatext"].fontsize, "THINOUTLINE")
...
local function UpdateCastbar(frame)
	...
	frame:SetSize(C["nameplate"].width, 5)
...
local function SkinObjects(frame, nameFrame)
	...
	hp.level:SetFont(C["media"].font, C["datatext"].fontsize, "THINOUTLINE")
	...
	if C["nameplate"].showhealth == true then
		...
		hp.value:SetFont(C["media"].font, C["datatext"].fontsize, "THINOUTLINE")
	...
	hp.name:SetFont(C["media"].font, C["datatext"].fontsize, "THINOUTLINE")
	...
	cb.time:SetFont(C["media"].font, C["datatext"].fontsize, "THINOUTLINE")
	...
	if C["nameplate"].showcastbarname == true then
		...
		cb.name:SetFont(C["media"].font, C["datatext"].fontsize, "THINOUTLINE")
...
local function ShowHealth(frame, ...)
	...
	if C["nameplate"].showhealth == true then
		frame.hp.value:SetText(D.ShortValue(valueHealth) .. " - " .. (string.format("%d%%", math.floor((valueHealth / maxHealth) * 100))))
	end
