...
local function LoadSkin()
	...
	for i=1, 7 do
		...
		frame.bg3:Point("TOPLEFT", D.mult*2, -D.mult*2)
		frame.bg3:Point("BOTTOMRIGHT", -D.mult*2, D.mult*2)			
	...
	for _, compare in pairs(compares) do
		for i=1, 9 do
			...
			_G[frame].bg1:Point("TOPLEFT", D.mult*4, -D.mult*4)
			_G[frame].bg1:Point("BOTTOMRIGHT", -D.mult*4, D.mult*4)				
			...
			_G[frame].bg3:Point("TOPLEFT", D.mult*2, -D.mult*2)
			_G[frame].bg3:Point("BOTTOMRIGHT", -D.mult*2, D.mult*2)			
	...
	hooksecurefunc("AchievementButton_GetProgressBar", function(index)
		...
		if frame then
			if not frame.skinned then
				...
				frame.bg1:Point("TOPLEFT", -D.mult*3, D.mult*3)
				frame.bg1:Point("BOTTOMRIGHT", D.mult*3, -D.mult*3)
				
				frame.bg2 = frame:CreateTexture(nil, "BACKGROUND")
				frame.bg2:SetDrawLayer("BACKGROUND", -6)
				frame.bg2:SetTexture(unpack(C["media"].bordercolor))
				frame.bg2:Point("TOPLEFT", -D.mult*2, D.mult*2)
				frame.bg2:Point("BOTTOMRIGHT", D.mult*2, -D.mult*2)					

				frame.bg3 = frame:CreateTexture(nil, "BACKGROUND")
				frame.bg3:SetDrawLayer("BACKGROUND", -5)
				frame.bg3:SetTexture(unpack(C["media"].backdropcolor))
				frame.bg3:Point("TOPLEFT", -D.mult, D.mult)
				frame.bg3:Point("BOTTOMRIGHT", D.mult, -D.mult)		
...
D.SkinFuncs["Blizzard_AchievementUI"] = LoadSkin