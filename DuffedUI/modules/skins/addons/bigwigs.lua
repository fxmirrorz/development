...
if not IsAddOnLoaded("BigWigs") or C["skins"].bigwigs ~= true then return end
...
local barcolor = classcolor and RAID_CLASS_COLORS[D.myclass]
local buttonsize

if C["actionbar"].buttonsize and type(C["actionbar"].buttonsize) =="number" then
	buttonsize = D.Scale(C["actionbar"].buttonsize - 2)
else
	buttonsize = 30
end
...
local applystyle = function(bar)
	...
	bg:Show()
	bar:Set("bigwigs:duffedui_bigwigs:bg", bg)
	...
	bar.candyBarLabel:SetFont(C["media"].font, C["datatext"].fontsize, "THINOUTLINE")
	bar.candyBarLabel:SetShadowColor(0, 0, 0, 0)
	bar.candyBarLabel:SetJustifyH("LEFT")
	bar.candyBarLabel:ClearAllPoints()
	bar.candyBarLabel:Point("BOTTOMLEFT", bar, "TOPLEFT", -2, 4)

	bar.candyBarDuration:SetFont(C["media"].font, C["datatext"].fontsize, "THINOUTLINE")
	...
	bar.candyBarBar.SetPoint = D.dummy
	...
	bar.candyBarIconFrame.SetWidth = D.dummy
...
local pr = function(msg)
	print("|cffC41F3BDuffedUI BigWigs Skin|r:", tostring(msg))
end

SLASH_DUFFEDUIBIGWIGS1 = "/dbigwigs"
SlashCmdList["DUFFEDUIBIGWIGS"] = function(msg)
	if msg == "apply" then
		SlashCmdList["BigWigs"]()
		HideUIPanel(InterfaceOptionsFrame)
		D.ShowPopup("DUFFEDUI_ENABLE_BIGWIGS_SKIN")
	elseif msg == "test" then
		...
		pr("use |cffC41F3B/dbigwigs apply|r to apply BigWigs settings.")
		pr("use |cffC41F3B/dbigwigs test|r to launch BigWigs testmode.")
	end
end

D.CreatePopup["DUFFEDUI_ENABLE_BIGWIGS_SKIN"] = {
	question = "Enable BigWigs-Skin",
	answer1 = ACCEPT,
	answer2 = CANCEL,
	function1 = function()
		BigWigs.pluginCore.modules.Bars.db.profile.barStyle = "DuffedUI_BigWigs"
