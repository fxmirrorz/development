...
local classcolor = RAID_CLASS_COLORS[D.myclass]
local buttonsize
		...
		if not bar.injected then
			bar.ApplyStyle = function()
				...
				if not timer.styled then
					...
					timer:SetFont(C["media"].font, C["datatext"].fontsize, "THINOUTLINE")
					timer:SetJustifyH("RIGHT")
					timer:SetShadowColor(0, 0, 0, 0)
					timer.SetFont = D.dummy
					timer.styled = true
				end
	...
	if not anchor.styled then
		local header = { anchor:GetRegions() }
			if header[1]:IsObjectType("FontString") then
				header[1]:SetFont(C["media"].font, C["datatext"].fontsize, "THINOUTLINE")
...
local pr = function(msg)
	print("|cffC41F3BDuffedUI DBM Skin|r:", tostring(msg))
end
...
SLASH_DUFFEDUIDBM1 = "/ddbm"
SlashCmdList["DUFFEDUIDBM"] = function(msg)
	if msg == "apply" then
		D.ShowPopup("DUFFEDUI_ENABLE_DBM_SKIN")
	elseif msg == "test" then
		DBM:DemoMode()
	elseif msg == "bh"then
		healthdemo()
	else
		pr("use |cffC41F3B/ddbm apply|r to apply DBM settings.")
		pr("use |cffC41F3B/ddbm test|r to launch DBM testmode.")
		pr("use |cffC41F3B/ddbm bh|r to show test BossHealth frame.")
	end
end

D.CreatePopup["DUFFEDUI_ENABLE_DBM_SKIN"] = {
