...
local function Shared(self, unit)
	...
	if (unit == "player" or unit == "target") then
		...
		health.value = D.SetFontString(panel, font1, 12)
		...
		power.value = D.SetFontString(panel, font1, 12)
		...
		if (unit == "player") then
			...
			FlashInfo.ManaLevel = D.SetFontString(FlashInfo, font1, 12)
			FlashInfo.ManaLevel:SetPoint("CENTER", panel, "CENTER", 0, 0)
			self.FlashInfo = FlashInfo
			
			-- pvp status text
			local status = D.SetFontString(panel, font1, 12)
			...
			if D.myclass == "DRUID" then
				...
				if C.unitframes.druidmushroombar then	
					...
					m:SetWidth((D.lowversion and 186) or 250)
					m:SetHeight(8)
			...
			if C.unitframes.mageclassbar and D.myclass == "MAGE" then
				...
				mb:SetWidth((D.lowversion and 186) or 250)
				mb:SetHeight(8)
			...
			if C.unitframes.classbar then
				if D.myclass == "DRUID" then
					...
					eclipseBar:SetScript("OnShow", function() D.DruidBarDisplay(self, false) end)
					eclipseBar:SetScript("OnHide", function() D.DruidBarDisplay(self, false) end)
				...
				if D.myclass == "WARLOCK" then
					...
					wb:SetWidth((D.lowversion and 186) or 250)
					wb:SetHeight(8)
				...
				if D.myclass == "DEATHKNIGHT" then
					...
					if C.unitframes.showstatuebar then
						...
						bar:SetWidth((D.lowversion and 186) or 250)
						bar:SetHeight(8)
				...
				if D.myclass == "MONK" then
					...
					hb:SetWidth((D.lowversion and 186) or 250)
					hb:SetHeight(8)
				...
				if D.myclass == "PRIEST" then
					...
					pb:SetWidth((D.lowversion and 186) or 250)
					pb:SetHeight(8)
				...
				if D.myclass == "SHAMAN" then
					...
					for i = 1, 4 do
						...
						elseif i == 1 then
							fixme = 62
							if D.lowversion then fixme = 46 end
		...
		if C["unitframes"].classiccombo and unit == "target" then
			...
			if (unit == "target" and D.myclass ~= "ROGUE") or (D.myclass == "ROGUE" and C.unitframes.movecombobar and unit == "player") or (D.myclass == "ROGUE" and not C.unitframes.movecombobar and unit == "target") then
				local CP = CreateFrame("Frame", "DuffedUICombo", self)
				CP:Point("BOTTOMLEFT", self, "TOPLEFT", 0, 1)
				CP:Width(D.lowversion and 186 or 250)
				CP:Height(8)
				CP:SetBackdrop(backdrop)
				CP:SetBackdropColor(0, 0, 0)
				CP:SetBackdropBorderColor(unpack(C["media"].backdropcolor))
				CP.PostUpdate = D.ComboPointsBarUpdate

				for i = 1, 5 do
					CP[i] = CreateFrame("StatusBar", "DuffedUIComboBar"..i, CP)
					CP[i]:Height(8)
					CP[i]:SetStatusBarTexture(normTex)
					
					if i == 1 then
						CP[i]:Point("LEFT", CP, "LEFT", 0, 0)
						if D.lowversion then CP[i]:Width(186 / 5 + 1) else CP[i]:Width(250 / 5) end
					else
						CP[i]:Point("LEFT", CP[i-1], "RIGHT", 1, 0)
						if D.lowversion then CP[i]:Width(186 / 5 - 1) else CP[i]:Width(250 / 5 - 1) end
					end					
				end
		...
		if (unit == "target" and C["unitframes"].targetauras) then
			...
			buffs.PostUpdate = D.UpdateTargetDebuffsHeader
			self.Buffs = buffs	
		...
		if (C["unitframes"].unitcastbar == true) then
			...
			castbar.time = D.SetFontString(castbar, font1, 12)
			castbar.time:Point("RIGHT", panel, "RIGHT", -4, 0)
			castbar.time:SetTextColor(0.84, 0.75, 0.65)
			castbar.time:SetJustifyH("RIGHT")

			castbar.Text = D.SetFontString(castbar, font1, 12)
		...
		if C["unitframes"].combatfeedback == true then
			...
			if D.lowversion then
				CombatFeedbackText = D.SetFontString(health, font1, 12, "OUTLINE")
			else
				CombatFeedbackText = D.SetFontString(health, font1, 14, "OUTLINE")
			end
	...
	if (unit == "targettarget") then
		...
		if C["unitframes"].totdebuffs == true and D.lowversion ~= true then
	...
	if (unit == "focus") then
		...
		health.value = D.SetFontString(health, font1,12, "OUTLINE")
		...
		power.value = D.SetFontString(health, font1, 12, "OUTLINE")
	...
	if(self:GetParent():GetName():match"DuffedUIMainTank" or self:GetParent():GetName():match"DuffedUIMainAssist") then
		...
		self.PostUpdateUnit = D.PostUpdateUnit or D.dummy
...
local adjust = 0
if D.lowversion then adjust = 125 end

-- for lower reso
local adjustXY = 0
local totdebuffs = 0
if D.lowversion then adjustXY = 24 end
...
player:SetPoint("BOTTOMLEFT", InvDuffedUIActionBarBackground, "TOPLEFT", 0,8+adjustXY)
...
focus:SetPoint("BOTTOMLEFT", InvDuffedUIActionBarBackground, "TOPLEFT", 0 - adjust, 300)
...
target:SetPoint("BOTTOMRIGHT", InvDuffedUIActionBarBackground, "TOPRIGHT", 0,8+adjustXY)
...
tot:SetParent(DuffedUIPetBattleHider)
if D.lowversion then
	tot:SetPoint("BOTTOMRIGHT", InvDuffedUIActionBarBackground, "TOPRIGHT", 0,8)
	tot:Size(186, 18)
else
	tot:SetPoint("BOTTOM", InvDuffedUIActionBarBackground, "TOP", 0,8)
	tot:Size(129, 36)
end
...
if D.lowversion then
	pet:SetPoint("BOTTOMLEFT", InvDuffedUIActionBarBackground, "TOPLEFT", 0,8)
	pet:Size(186, 18)
else
	pet:SetPoint("BOTTOM", InvDuffedUIActionBarBackground, "TOP", 0,49+totdebuffs)
	pet:Size(129, 36)
end
...
if C.unitframes.arena then
	...
	for i = 1, 5 do
		...
		if i == 1 then
			arena[i]:SetPoint("BOTTOMRIGHT", InvDuffedUIActionBarBackground, "TOPRIGHT", 0 + adjust, 300)
		else
...
if C["unitframes"].showboss then
	...
	for i = 1, MAX_BOSS_FRAMES do
		...
		if i == 1 then
			boss[i]:SetPoint("BOTTOMRIGHT", InvDuffedUIActionBarBackground, "TOPRIGHT", 0 + adjust, 300)
		else
...
if C.unitframes.raid == true then
	...
	local function Shared(self, unit)
		...
		self:SetBackdrop({bgFile = C["media"].blank, insets = {top = -D.mult, left = -D.mult, bottom = -D.mult, right = -D.mult}})
		self:SetBackdropColor(0, 0, 0)
		
		local health = CreateFrame("StatusBar", nil, self)
		health:SetPoint("TOPLEFT")
		health:SetPoint("TOPRIGHT")
		health:Height(28*C["unitframes"].gridscale*D.raidscale)
		health:SetStatusBarTexture(normTex)
		self.Health = health
		...
		health.value:SetFont(font2, 11*C["unitframes"].gridscale*D.raidscale, "THINOUTLINE")
		...
		local power = CreateFrame("StatusBar", nil, self)
		power:SetHeight(3*C["unitframes"].gridscale*D.raidscale)
		...
		name:SetPoint("LEFT") 
		name:SetPoint("RIGHT")
		name:SetFont(font2, 12*C["unitframes"].gridscale*D.raidscale)
		self:Tag(name, "[DuffedUI:getnamecolor][DuffedUI:nameshort]")
		self.Name = name
		...
		if C["unitframes"].showsymbols == true then
			...
			RaidIcon:SetTexture("Interface\\AddOns\\DuffedUI\\medias\\textures\\raidicons.blp") -- thx hankthetank for texture
			RaidIcon.SetTexture = D.dummy -- idk why but RaidIcon:GetTexture() is returning nil in oUF, resetting icons to default ... stop it!
			self.RaidIcon = RaidIcon
		end
		...
		LFDRole.Override = D.SetGridGroupRole
		...
		--picon:SetSize(16, 16)
		--picon:SetTexture[[Interface\AddOns\DuffedUI\medias\textures\picon]]
		--picon.Override = D.Phasing
		...
		if C["unitframes"].healcomm then
			...
			if C["unitframes"].gridhealthvertical then
				...
				mhpb:Width(66*C["unitframes"].gridscale*D.raidscale)
				mhpb:Height(50*C["unitframes"].gridscale*D.raidscale)		
			else
				mhpb:SetPoint("TOPLEFT", self.Health:GetStatusBarTexture(), "TOPRIGHT", 0, 0)
				mhpb:SetPoint("BOTTOMLEFT", self.Health:GetStatusBarTexture(), "BOTTOMRIGHT", 0, 0)
				mhpb:Width(66*C["unitframes"].gridscale*D.raidscale)
			...
			if C["unitframes"].gridhealthvertical then
				...
				ohpb:Width(66*C["unitframes"].gridscale*D.raidscale)
				ohpb:Height(50*C["unitframes"].gridscale*D.raidscale)
			else
				ohpb:SetPoint("TOPLEFT", mhpb:GetStatusBarTexture(), "TOPRIGHT", 0, 0)
				ohpb:SetPoint("BOTTOMLEFT", mhpb:GetStatusBarTexture(), "BOTTOMRIGHT", 0, 0)
				ohpb:Width(6*C["unitframes"].gridscale*D.raidscale)
			end
			ohpb:SetStatusBarTexture(normTex)
			ohpb:SetStatusBarColor(0, 1, 0, 0.25)
		...
		self.PostUpdateRaidUnit = D.PostUpdateRaidUnit or D.dummy
	...
	local y = D.screenheight / 9
	
	oUF:RegisterStyle("DuffedUIRaid", Shared)
	...
	local function GetAttributes()
		...
		"initial-width", D.Scale(66*C["unitframes"].gridscale*D.raidscale),
		"initial-height", D.Scale(50*C["unitframes"].gridscale*D.raidscale),
		"showParty", true,
		"showRaid", true,
		"showPlayer", true,
	...
	D.RaidFrameAttributes = GetAttributes
	
	if C.unitframes.showraidpets then
		local function GetPetAttributes()
			return
			"DuffedUIRaidPet", "SecureGroupPetHeaderTemplate", "solo,party,raid",
			"showPlayer", true,
			"showParty", true,
			"showRaid", true,
		...
		D.RaidFramePetAttributes = GetPetAttributes
	end
	...
	oUF:Factory(function(self)
		...
		if C["unitframes"].maintank == true then
			...
			D.MainTankAttributes = GetAttributes
			
			local tank = oUF:SpawnHeader(D.MainTankAttributes())
		...
		if C["unitframes"].mainassist == true then
			...
			D.MainAssistAttributes = GetAttributes
			
			local assist = oUF:SpawnHeader(D.MainAssistAttributes())
		...
		if D.isAltRaidFrame then return end
		
		oUF:SetActiveStyle("DuffedUIRaid")

		local raid = self:SpawnHeader(D.RaidFrameAttributes())
		raid:SetParent(DuffedUIPetBattleHider)
		raid:Point("TOPLEFT", UIParent, "TOPLEFT", 18, -y)
		G.UnitFrames.RaidUnits = raid
	
		if C.unitframes.showraidpets then
			local pet = self:SpawnHeader(D.RaidFramePetAttributes())
			pet:SetParent(DuffedUIPetBattleHider)
		...
		if C.unitframes.maxraidplayers then
			...
			local MaxGroup = CreateFrame("Frame", "DuffedUIRaidMaxGroup")
			...
			MaxGroup:SetScript("OnEvent", function(self)
				...
				if inInstance and instanceType == "raid" then
					DuffedUIRaid:SetAttribute("groupFilter", filter)
					if C.unitframes.showraidpets then
						DuffedUIRaidPet:SetAttribute("groupFilter", filter)
					end
				else
					DuffedUIRaid:SetAttribute("groupFilter", "1,2,3,4,5,6,7,8")
					if C.unitframes.showraidpets then
						DuffedUIRaidPet:SetAttribute("groupFilter", "1,2,3,4,5,6,7,8")
					end
				end
			end)
		end
